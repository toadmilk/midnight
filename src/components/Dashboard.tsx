  ...
  return (
    <main className="mx-auto max-w-7xl md:p-10">
      ...
      {files && files?.length !== 0 ? (
        <ul className="mt-8 grid grid-cols-1 gap-6 divide-y divide-neutral-200 dark:divide-neutral-800 md:grid-cols-2 lg:grid-cols-3">
          {files.sort(
            (a, b) =>
              new Date(b.createdAt).getTime() -
              new Date(a.createdAt).getTime(),
          ).map((file) => (
            <li key={file.id} className="col-span-1 divide-y divide-neutral-200 dark:divide-neutral-800 rounded-lg bg-background shadow transition hover:shadow-lg">
